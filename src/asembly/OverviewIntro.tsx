...
import { z } from "zod";
import { FingerTap, FONT, PhoneAssetFrame, PhoneBubble, PhoneFrame, ScanView } from "./shared";
...
export const overviewIntroSchema = z.object({
  時間軸: z.object({
    ...
    情境手機: 軌(270, 588, "情境段連續在場的手機（螢幕隨各情境切換）"),
    情境一找休息: 軌(270, 315, "地圖頁＋服務區示意卡"),
    情境二下一站: 軌(315, 360, "下一站推薦頁＋維修展演卡"),
    情境三展演提醒: 軌(360, 405, "展演時刻頁＋DT668 卡"),
    情境四搜尋: 軌(405, 588, "搜尋操作實錄影片：輸入車削→點互動機具→展品頁（手機滑至中央）"),
    黑幕淡出: 軌(588, 618, "淡黑接第一支功能片；結束幀＝影片總長"),
  }),
  文案: z.object({
...
const SmallCard: React.FC<{ title: string; subtitle: string; enterFrame: number; maxWidth?: number }> = ({ title, subtitle, enterFrame, maxWidth = 900 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const p = spring({ frame: frame - enterFrame, fps, config: { damping: 200 } });
  return (
    <div style={{
      position: "absolute", top: 56, left: 64, maxWidth,
...
const STEP_SCREEN_SRCS = ["map.png", "audience.png", "schedule.png"] as const;
...
const StepScreens: React.FC<{ bounds: number[] }> = ({ bounds }) => {
  ...
  return <>
    {STEP_SCREEN_SRCS.map((src, i) => {
      ...
      const fadeOut = interpolate(frame, [bounds[i + 1], bounds[i + 1] + 6], [1, 0], clamp);
    ...
    {/* 情境四：導覽原型實錄（輸入車削→點「互動機具」→展品頁），與軌同起點播放 */}
    <Sequence name="搜尋操作影片" from={bounds[3]} durationInFrames={bounds[4] - bounds[3]}>
      <OffthreadVideo muted trimBefore={24} src={staticFile(OVERVIEW("search_flow.mp4"))} style={{ position: "absolute", width: "100%", height: "100%", objectFit: "cover" }} />
    </Sequence>
  </>;
};
    ...
    <Sequence name="情境手機" from={T.情境手機.開始} durationInFrames={dur(T.情境手機)}>
      <TrackFade len={dur(T.情境手機)}>
        <PhoneFrame enterFrame={2} x={phoneX}
          overlay={<FingerTap src={AIR("finger_tap_po.png")} tip={[881, 161]} imgSize={1024} scale={0.87}
            target={[210, 516]} start={T.情境四搜尋.開始 - T.情境手機.開始 + 72} tapAt={T.情境四搜尋.開始 - T.情境手機.開始 + 102}
            end={T.情境四搜尋.開始 - T.情境手機.開始 + 160} from={[-560, 500]} />}>
    ...
    {情境軌.map((s, i) => (
      <Sequence key={i} name={["情境一找休息", "情境二下一站", "情境三展演提醒", "情境四搜尋"][i]} from={s.開始} durationInFrames={dur(s)}>
        <TrackFade len={dur(s)}>
          ...
          <SmallCard title={文案.情境卡[i].標題} subtitle={文案.情境卡[i].副標} enterFrame={2} maxWidth={330} />
        </TrackFade>
      </Sequence>
    ))}
